...
[HarmonyPatch(typeof(MapBehaviour), nameof(MapBehaviour.ShowNormalMap))]
	public static class Minimap_ShowMapPostfix
	{
		public static bool isActive(){
			return CheatSettings.mapCrew || CheatSettings.mapGhosts || CheatSettings.mapImps;
		}
		...
		public static void Postfix(MapBehaviour __instance)
		{
			...
			foreach (var herePoint in temp)
			{
				try{ //try-catch to fix issues caused by player disconnection
					...
					//Crewmate, alive
					if (CheatSettings.mapCrew && !herePoint.player.Data.Role.IsImpostor){
						if (!herePoint.player.Data.IsDead){
							herePoint.sprite.gameObject.SetActive(true);
							if (CheatSettings.colorBasedMap){
								color = herePoint.player.Data.Color; //Color-Based Icon
							}else{
								color = herePoint.player.Data.Role.TeamColor; //Role-Based Icon
							}
						}

					//Impostor, alive
					} else if (CheatSettings.mapImps && herePoint.player.Data.Role.IsImpostor){
						if (!herePoint.player.Data.IsDead){
							herePoint.sprite.gameObject.SetActive(true);
							if (CheatSettings.colorBasedMap){
								color = herePoint.player.Data.Color; //Color-Based Icon
					...
					//Any Role, dead
					if (CheatSettings.mapGhosts && herePoint.player.Data.IsDead){
						herePoint.sprite.gameObject.SetActive(true);
						if (CheatSettings.colorBasedMap){
							color = herePoint.player.Data.Color; //Color-Based Icon
						}else{
