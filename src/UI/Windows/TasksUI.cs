...
public class TasksUI : MonoBehaviour
{
    ...
    {
        if (
            !CheatToggles.showTasksMenu
            || !(MenuUI.isGUIActive || MalumMenu.Settings.MenuKeepSubwindowsOpen.Value)
            || MalumMenu.IsPanicked
        )
            return;
        ...
        _windowRect = GUI.Window(
            (int)WindowId.TasksUI,
            _windowRect,
            (GUI.WindowFunction)TasksWindow,
            "Tasks"
        );
    }
    ...
    private void TasksWindow(int windowID)
    {
        ...
        foreach (var player in PlayerControl.AllPlayerControls)
        {
            ...
            if (
                GUILayout.Button(
                    $"{arrow} [{completeCount}/{taskCount}] <color=#{ColorUtility.ToHtmlStringRGB(player.Data.Color)}>{nameKey}</color>",
                    _playerHeaderStyle
                )
            )
            {
            ...
            if (expanded)
            {
                ...
                foreach (var task in player.myTasks)
                {
                    ...
                    if (taskText.Contains("You're dead") || taskText.Contains("Sabotage and kill"))
                        continue;
