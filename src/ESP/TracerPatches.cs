    ...
    public static void Postfix(PlayerPhysics __instance)
    {
        try // try-catch to avoid errors in lobbies
        {
            if (!__instance.AmOwner) // Don't draw a tracer for LocalPlayer
            {
                ...
                else if (CheatToggles.tracersImps && __instance.myPlayer.Data.Role.IsImpostor) // Alive Impostor
                {
                    if (!__instance.myPlayer.Data.IsDead)
                    {
                        if (CheatToggles.colorBasedTracers)
                        {
                            color = __instance.myPlayer.Data.Color;
                        }
                        else
                        {
                            color = __instance.myPlayer.Data.Role.TeamColor;
                        }
                    }
                }

                // Dead Player (any role)
    ...
    public static void Postfix(PlayerPhysics __instance)
    {
        ...
        foreach (GameObject bodyObject in bodyObjects) // Finds and loops through all dead bodies
        {
            ...
            if (deadBody)
            {
                ...
                { // Only unreported dead bodies have tracers (other bodies are hidden)
                    ...
                    if (CheatToggles.tracersBodies)
                    {
                        ...
                        {
                            color = Color.yellow; // Dead body color
                        }
                    }
